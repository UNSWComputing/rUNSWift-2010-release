...
#include "alptr.h"
#include "allauncherproxy.h"

using namespace std;

// maps for mapping threads to jump points and error
ConcurrentMap<pthread_t, jmp_buf*> jumpPoints;
ConcurrentMap<pthread_t, string>  threadNames;
...
void handleSignals(int sigNumber, siginfo_t* info, void*) {
   ...
   if ( sigNumber == SIGINT ) {
      ...
      cerr <<  threadNames[thread] << " with id " << thread <<
         " received signal " << sigNumber << " and is restarting" << endl;
      llog(ERROR) << threadNames[thread] << " with id "
