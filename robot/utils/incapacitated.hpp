...
inline bool isIncapacitated(const int &player) {
   bool incapacitated = false;
   if (time(NULL) - readFrom(receiver, lastReceived)[player-1]
         > SECS_TILL_INCAPACITATED) {
      incapacitated = true;
   }

   if (readFrom(gameController, our_team).players[player-1].penalty
         != PENALTY_NONE) {
      incapacitated = true;
   }

   const ActionCommand::Body::ActionType &acB =
      readFrom(receiver, data)[player-1].acB;
